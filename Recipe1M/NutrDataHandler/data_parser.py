...
def ing_parser(ingredients, units, quantities, nutr_info):
    ...
    for index, ing in enumerate(ingredients):
        ...
        protein_of_ing = float(nutr_info[index]['pro'])
        unit_protein = protein_of_ing / quantity if quantity else 0
        fat_of_ing = float(nutr_info[index]['fat'])
        unit_fat = fat_of_ing / quantity if quantity else 0
        sugar_of_ing = float(nutr_info[index]['sug'])
        unit_sugar = sugar_of_ing / quantity if quantity else 0
        
        # Append to the sorted ingredient and unit lists
        append_sorted(ing, unit, '{:.2f}'.format(unit_calories), '{:.2f}'.format(unit_protein), '{:.2f}'.format(unit_fat), '{:.2f}'.format(unit_sugar))
...
def process_apply(row):
    ...
    quantities = [parse_fraction(quantity) for quantity in ast.literal_eval(row['quantity'])]
    nutr_info = ast.literal_eval(row['nutr_per_ingredient'])
    weights = ast.literal_eval(row['weight_per_ingr'])
...
def parser_main():
    ...
    data_dir_path = os.path.join(path_of_the_current_scripts_dir, '../', "nutr_partitioned_data")
